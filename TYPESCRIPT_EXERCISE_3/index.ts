import { studentInformation } from "./calculation";
interface Student {
    name: string;
    age?: number;
    rank: number | string;
...
const fetchStudentDetails = <T extends { rank: number | string }>(studentDetails: T[]): T[] => {
    let filteredStudents = studentDetails.filter((students) => students.rank !== "");
    let studentRank = filteredStudents.map((students) => ({
        ...students,
        grade: studentInformation.fetchGrade(Number(students.rank))
    }));
    return studentRank;
};
let resultRank = fetchStudentDetails(studentDetails);
let result = studentInformation.studentPerformance(resultRank);
